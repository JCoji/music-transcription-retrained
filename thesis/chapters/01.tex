...
Automatyczna Transkrypcja Muzyczna (ATM) (ang. \textit{Automatic Music Transcription}) stanowi jedno z kluczowych i jednocześnie najbardziej złożonych zadań w dziedzinie komputerowej analizy muzyki.
...
\begin{itemize}
    \item \textbf{Cyfrowe Przetwarzanie Sygnałów (CPS)} (ang. \textit{Digital Signal Processing – DSP}): Dostarcza ono fundamentalnych narzędzi do analizy, transformacji i manipulacji sygnałem audio w dziedzinie czasu i częstotliwości.
    ...
    Przykładem fundamentalnego narzędzia CPS, ilustrującego przejście z dziedziny czasu do dziedziny częstotliwości, jest dyskretna transformata Fouriera (DFT) (ang. \textit{Discrete Fourier Transform}).
    ...
    \item \textbf{Rozpoznawanie Wzorców} (ang. \textit{Pattern Recognition}) oraz \textbf{Uczenie Maszynowe (UM)} (ang. \textit{Machine Learning – ML}): Te dziedziny odgrywają kluczową rolę w identyfikacji złożonych struktur muzycznych, takich jak pojedyncze nuty, akordy, rytmy czy motywy melodyczne, w często zaszumionym i polifonicznym sygnale audio \autocite{Benetos2019AutomaticMusicTranscription}.
    Współczesne systemy ATM w coraz większym stopniu opierają się na zaawansowanych modelach uczenia maszynowego, w szczególności na sieciach neuronowych i głębokim uczeniu (ang. \textit{Deep Learning}), które potrafią automatycznie uczyć się hierarchicznych reprezentacji cech muzycznych bezpośrednio z danych \autocite{BurletHindle2017IsolatedGuitarTranscription}. Przełomem w tej dziedzinie okazał się paradygmat ,,Onsets and Frames'', zaproponowany w pracy \autocite{Hawthorne2018Onsets}, w którym model uczy się jednocześnie detekcji początków nut (onsetów) oraz ich trwania (predykcji na poziomie ramek), co znacząco poprawiło jakość transkrypcji i stało się podstawą dla wielu nowoczesnych systemów, w tym rozwiązania rozwijanego w niniejszej pracy.
    ...
    \item \textbf{Sztuczna Inteligencja (SI)} (ang. \textit{Artificial Intelligence – AI}): Jako szersza dziedzina, obejmuje ona uczenie maszynowe, ale także inne aspekty, takie jak reprezentacja wiedzy muzycznej oraz procesy poznawcze związane z percepcją i wnioskowaniem w kontekście muzycznym \autocite{Benetos2019AutomaticMusicTranscription}.
...
Przedmiotem niniejszej pracy jest zagadnienie automatycznej transkrypcji muzyki gitarowej do postaci tabulatury, co sytuuje ją w szerokim kontekście badań prowadzonych w ramach dziedziny naukowej znanej jako (ang. \textit{Music Information Retrieval (MIR)}).
...
\begin{itemize}
    ...
    \end{figure}
    \item \textbf{Bogactwo technik artykulacyjnych}: Gitarzyści wykorzystują szeroki wachlarz technik artykulacyjnych, takich jak podciąganie strun (ang. \textit{bending}), ślizgi (ang. \textit{sliding}), młoteczkowanie (ang. \textit{hammer-on}), odrywanie (ang. \textit{pull-off}), wibrato (ang. \textit{vibrato}) czy tłumienie strun dłonią (ang. \textit{palm-muting}).
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Moduł przetwarzania wstępnego sygnału audio (ang. \textit{audio preprocessing}), mający na celu przygotowanie sygnału do dalszej analizy, np. poprzez normalizację, segmentację czy redukcję szumów.
        \item Moduł ekstrakcji cech akustycznych (ang. \textit{acoustic feature extraction}), odpowiedzialny za przekształcenie przetworzonego sygnału audio w reprezentację numeryczną, która efektywnie koduje informacje istotne dla rozpoznawania dźwięków gitarowych i ich atrybutów.
        ...
        \item Moduł predykcyjny, oparty na wybranych technikach uczenia maszynowego, którego zadaniem będzie generowanie sekwencji zdarzeń tabulaturowych (struna, próg, czas rozpoczęcia, czas trwania) na podstawie wyekstrahowanych cech akustycznych.
    \end{itemize}
    ...
    Wyniki uzyskane przez system będą porównywane z adnotacjami referencyjnymi ze zbioru Guitarset oraz, w miarę możliwości, z wynikami innych istniejących metod lub systemów transkrypcji gitarowej, aby ocenić postęp względem aktualnego stanu wiedzy (ang. \textit{state-of-the-art}) \autocite{Benetos2019AutomaticMusicTranscription}.
...
\begin{description}
    ...
    Osadza problem w dziedzinie naukowej (ang. \textit{Music Information Retrieval (MIR)}), omawia wyzwania związane z transkrypcją gitarową.
...
\end{description}
Przedstawiona struktura pracy jest zgodna ze standardami pisania prac naukowo-badawczych w dziedzinach technicznych, co zapewnia jej spójność i kompletność.
