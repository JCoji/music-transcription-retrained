...
class GuitarSetTabDataset(Dataset):
    ...
    def __getitem__(self, item_idx):
        if item_idx >= len(self.feature_sources):
            raise IndexError(
                f"Indeks {item_idx} poza zakresem {len(self.feature_sources)}"
            )
        ...
        if self.enable_audio_augmentations:
            ...
            if sr_loaded != self.audio_sample_rate:
                audio_data = librosa.resample(
                    y=audio_data, orig_sr=sr_loaded, target_sr=self.audio_sample_rate
                )
            ...
            if np.random.rand() < self.aug_p_time_stretch:
                ...
                audio_data, current_labels_for_stretch = _augment_time_stretch(
                    audio_data, stretch_factor_val, current_labels_for_stretch
                )
    ...
    def get_full_track_id_for_item(self, item_idx):
        if item_idx >= len(self.full_track_ids):
            raise IndexError(
                f"Indeks {item_idx} poza zakresem listy full_track_ids ({len(self.full_track_ids)})"
            )
