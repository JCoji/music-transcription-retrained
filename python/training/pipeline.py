...
import config
from evaluation import performance_metrics, tablature_export, midi_export
...
def run_training_loop(
    ...
    fret_classes_count = run_training_config.get("FRET_NUM_CLASSES",
                                                 config.MAX_FRETS + config.FRET_SILENCE_CLASS_OFFSET + 1)
    artifacts_output_dir = run_training_config.get("ARTIFACTS_DIR", "training_artifacts")
    training_log_filepath = run_training_config.get("LOG_FILE_PATH",
                                                    os.path.join(artifacts_output_dir, "training_log.txt"))

    clear_console_output_every_n = run_training_config.get("CLEAR_CONSOLE_EVERY_N_EPOCHS_WITHIN_RUN", 0)
    clear_output_function_ref = run_training_config.get("CLEAR_OUTPUT_FUNC", None)

    # Usunięto plot_preds_every_n_epochs
    fixed_thresh_for_log = run_training_config.get("ONSET_PREDICTION_THRESHOLD", config.DEFAULT_ONSET_THRESHOLD)

    early_stop_patience_val = run_training_config.get("EARLY_STOPPING_PATIENCE", config.EARLY_STOPPING_PATIENCE_DEFAULT)
    checkpoint_metric_to_track = run_training_config.get("CHECKPOINT_METRIC",
                                                         config.CHECKPOINT_METRIC_DEFAULT)  # Np. "val_tdr_recall"

    best_tracked_metric_val = -float("inf")
    if "loss" in checkpoint_metric_to_track.lower():  # Bardziej ogólne sprawdzenie
        best_tracked_metric_val = float("inf")
    ...
    training_history = {
        ...
        "onset_f1_at_fixed_thresh_frame": [], "onset_precision_at_fixed_thresh_frame": [],
        # Zmieniono nazwy dla jasności
        "onset_recall_at_fixed_thresh_frame": [], "onset_accuracy_at_fixed_thresh_frame": [],

        "val_onset_f1_optimal_thresh_frame": [], "val_onset_precision_optimal_thresh_frame": [],  # Zmieniono nazwy
        "val_onset_recall_optimal_thresh_frame": [], "val_onset_accuracy_optimal_thresh_frame": [],
        "val_optimal_threshold_epoch_frame": [],  # Ten próg jest dla metryk ramkowych

        "val_ftab": [],
        "val_onset_precision_mir_eval": [], "val_onset_recall_mir_eval": [], "val_onset_f1_mir_eval": [],
        "val_tdr_precision": [], "val_tdr_recall": [], "val_tdr_f1": [],
        "lr": [],
    }
    ...
    with open(training_log_filepath, "a", encoding="utf-8") as log_file_handle:
        ...
        log_file_handle.write(f"--- Konfiguracja Treningu (tracking: {checkpoint_metric_to_track}) ---\n")
        for conf_key, conf_value in run_training_config.items():
            log_line = f"  {conf_key}: Function provided\n" if callable(
                conf_value) and conf_key == "CLEAR_OUTPUT_FUNC" else f"  {conf_key}: {conf_value}\n"
            log_file_handle.write(log_line)
        log_file_handle.write("-" * 30 + "\n\n")

        print(
            f"\nRozpoczynanie pętli treningowej na {num_epochs_total} epok. Śledzona metryka: {checkpoint_metric_to_track}")
        for current_epoch_num in range(num_epochs_total):
            epoch_description_str = f"Epoka {current_epoch_num + 1}/{num_epochs_total}"

            if (clear_output_function_ref and clear_console_output_every_n > 0 and
                    current_epoch_num > 0 and (current_epoch_num) % clear_console_output_every_n == 0):
                clear_output_function_ref(wait=True)
                print(f"Output wyczyszczony. Kontynuacja treningu ({epoch_description_str})...")

            train_pbar = tqdm(train_loader, desc=f"{epoch_description_str} [Trening]", unit="batch", leave=False,
                              dynamic_ncols=True)
            train_epoch_metrics = epoch_processing.train_one_epoch(
                model_instance, train_pbar, optimizer_instance, criterion_instance_combined, device_to_use
            )
            train_pbar.close()
            training_history["train_total_loss"].append(train_epoch_metrics["train_total_loss"])
            training_history["train_onset_loss"].append(train_epoch_metrics["train_onset_loss"])
            training_history["train_fret_loss"].append(train_epoch_metrics["train_fret_loss"])

            val_pbar = tqdm(validation_loader, desc=f"{epoch_description_str} [Walidacja]", unit="batch", leave=False,
                            dynamic_ncols=True)
            # Zmieniono nazwę zwracanego słownika dla jasności
            val_epoch_all_metrics, val_epoch_onset_probs_agg, val_epoch_onset_targets_agg = epoch_processing.evaluate_one_epoch(
                model_instance, val_pbar, criterion_instance_combined, device_to_use,
                fret_classes_count,  # Przekazujemy poprawną liczbę klas
                audio_sr_val=audio_sr_val,  # Przekazujemy SR
                audio_hop_len_val=audio_hop_len_val,  # Przekazujemy hop_length
                log_fixed_onset_threshold=fixed_thresh_for_log
            )
            val_pbar.close()

            # Zapisywanie wszystkich metryk zwróconych przez evaluate_one_epoch
            for key, value in val_epoch_all_metrics.items():
                if key in training_history:
                    training_history[key].append(value)
                else:  # Jeśli klucz nie istnieje (np. nowa metryka), dodaj go
                    training_history[key] = [value]

                    # Jeśli 'val_optimal_threshold_epoch_frame' nie został zwrócony (bo np. nie było danych)
            if "val_optimal_threshold_epoch_frame" not in val_epoch_all_metrics and "val_optimal_threshold_epoch_frame" in training_history:
                training_history["val_optimal_threshold_epoch_frame"].append(config.DEFAULT_ONSET_THRESHOLD)
            ...
            for line in log_lines_for_file[1:]: print(line)

            metric_value_for_checkpoint = val_epoch_all_metrics.get(checkpoint_metric_to_track,
                                                                    -float(
                                                                        "inf") if "loss" not in checkpoint_metric_to_track else float(
                                                                        "inf"))

            if scheduler_instance:
                if isinstance(scheduler_instance, torch.optim.lr_scheduler.ReduceLROnPlateau):
            ...
            if is_improved:
                epochs_without_improvement = 0
                model_checkpoint_path = os.path.join(artifacts_output_dir, "best_model.pth")
            ...
            elif early_stop_patience_val is not None and current_epoch_num > 0:
                ...
                if epochs_without_improvement == 1 or epochs_without_improvement % max(1,
                                                                                       early_stop_patience_val // 3) == 0:
                ...
                if epochs_without_improvement >= early_stop_patience_val:
                    ...
                    # Usunięto rysowanie predykcji co N epok
    ...
    # Rysowanie historii treningu na końcu
    if training_history:
        history_plot_path = os.path.join(artifacts_output_dir, "training_history_summary.png")
        if hasattr(plotting, 'plot_training_history'):
            print(f"Generowanie podsumowania historii treningu do: {history_plot_path}")
            plotting.plot_training_history(training_history, output_save_path=history_plot_path)
        else:
            print("Funkcja plot_training_history nie jest dostępna w visualization.plotting.")
...
def process_single_hyperparameter_run(
    ...
    current_run_folder_name_sanitized = f"run_{run_id}_{run_description_str.replace(' ', '_').replace('/', '-').replace(':', '').replace(',', '')[:50]}"

    current_run_artifacts_dir = os.path.join(main_artifacts_dir, current_run_folder_name_sanitized)
    ...
    print(f"--- Konfiguracja dla przebiegu {run_id} ({current_run_folder_name_sanitized}) ---")
    ...
    try:
        ...
        with torch.no_grad():
            ...
            calculated_cnn_out_dim = temp_cnn_model.output_channels * dummy_cnn_output.shape[2]
        ...
        error_summary = {"run_index": run_id, "params_combo": hyperparams_combo, "status": "CNN_DIM_ERROR", "error": str(e_cnn), "run_folder_name": current_run_folder_name_sanitized}
        return error_summary

    if calculated_cnn_out_dim is None:
        return {"run_index": run_id, "params_combo": hyperparams_combo, "status": "CNN_DIM_CALC_FAILED", "error": "calculated_cnn_out_dim is None", "run_folder_name": current_run_folder_name_sanitized}
    ...
    onset_pos_weight_tensor = torch.tensor([hyperparams_combo['ONSET_POS_WEIGHT_MANUAL_VALUE']], device=device) \
                              if hyperparams_combo['ONSET_POS_WEIGHT_MANUAL_VALUE'] > 0 else None
    ...
    optimizer = optim.AdamW(current_model.parameters(),
                            lr=hyperparams_combo['LEARNING_RATE_INIT'],
                            weight_decay=hyperparams_combo['WEIGHT_DECAY'])

    scheduler_mode = 'min' if 'loss' in config_obj.CHECKPOINT_METRIC_DEFAULT.lower() else 'max'
    ...
    training_run_configuration = {
        ...
        "FRET_NUM_CLASSES": config_obj.MAX_FRETS + config_obj.FRET_SILENCE_CLASS_OFFSET + 1,
        "ARTIFACTS_DIR": current_run_artifacts_dir,
        "LOG_FILE_PATH": current_run_log_file,
        "CLEAR_CONSOLE_EVERY_N_EPOCHS_WITHIN_RUN": config_obj.CLEAR_CONSOLE_EVERY_N_EPOCHS_WITHIN_RUN,
    ...
    print(f"Rozpoczynanie treningu dla przebiegu {run_id}. Logi w: {current_run_log_file}")
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        error_summary = {"run_index": run_id, "params_combo": hyperparams_combo,
    ...
    print(f"Zakończono trening dla przebiegu {run_id}. Czas (trening): {run_duration_training_minutes:.2f} min.")
    ...
    if training_run_history:
        ...
        tracked_metric_history = training_run_history.get(config_obj.CHECKPOINT_METRIC_DEFAULT, [])
        if tracked_metric_history:
            if "loss" in config_obj.CHECKPOINT_METRIC_DEFAULT.lower():
                best_val_metric_final = min(tracked_metric_history)
                best_metric_epoch_idx = np.argmin(tracked_metric_history)
            else:
                best_val_metric_final = max(tracked_metric_history)
                best_metric_epoch_idx = np.argmax(tracked_metric_history)

            if best_metric_epoch_idx != -1 and \
               "val_optimal_threshold_epoch_frame" in training_run_history and \
               len(training_run_history["val_optimal_threshold_epoch_frame"]) > best_metric_epoch_idx :
                  optimal_threshold_at_best_metric_frame = training_run_history["val_optimal_threshold_epoch_frame"][best_metric_epoch_idx]

    final_test_metrics = "N/A"
    final_loaded_model = None

    if test_loader:
        best_model_path_for_run = os.path.join(current_run_artifacts_dir, "best_model.pth")
        if os.path.exists(best_model_path_for_run):
            ...
            final_loaded_model = utils.load_best_model(
                current_model, model_init_parameters, best_model_path_for_run, device
            )
            if final_loaded_model:
                print(f"Ewaluacja na zbiorze testowym z progiem onsetów (ramkowym): {optimal_threshold_at_best_metric_frame:.2f}")
                final_test_metrics = performance_metrics.evaluate_model_on_test_set(
                     model_to_eval=final_loaded_model,
                     test_dataloader=test_loader,
                     device_to_use=device,
                     fret_num_classes_val=config_obj.MAX_FRETS + config_obj.FRET_SILENCE_CLASS_OFFSET + 1,
                ...
                print(f"Nie udało się załadować najlepszego modelu dla przebiegu {run_id}.")
        else:
            print(f"Nie znaleziono pliku best_model.pth dla przebiegu {run_id}.")
    else:
        print("Test DataLoader nie jest dostępny. Pomijam ewaluację na zbiorze testowym.")
    ...
    # Cleanup
    del current_model, optimizer, scheduler, combined_loss_criterion
    if training_run_history: del training_run_history
    if final_loaded_model: del final_loaded_model
